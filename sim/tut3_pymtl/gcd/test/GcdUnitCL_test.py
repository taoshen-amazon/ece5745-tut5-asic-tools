...
from pymtl3             import *
from pymtl3.stdlib.test import TestMasterCL, mk_test_case_table, run_sim
...
class TestHarness ( Component ):
  ...
  def construct( s, gcd ):
    ...
    s.tm  = TestMasterCL( GcdUnitMsgs.req, GcdUnitMsgs.resp )
    s.gcd = gcd

    # Connect

    s.tm.master //= s.gcd.minion

  def done( s ):
    return s.tm.done()

  def line_trace( s ):
    return s.tm.line_trace() + " > " + s.gcd.line_trace()
