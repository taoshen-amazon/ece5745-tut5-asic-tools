...
class SortUnitFlatRTL( Component ):
  ...
  def construct( s, nbits=8 ):
    ...
    s.val_S1 = Wire()
    s.elm_S1 = [ Wire(DataType) for _ in range(4) ]

    @s.update_ff
    def pipereg_S0S1():
      s.val_S1 <<= s.in_val if ~s.reset else b1(0)
      for i in range(4):
        s.elm_S1[i] <<= s.in_[i]
    ...
    s.elm_next_S1 = [ Wire(DataType) for _ in range(4) ]
    ...
    s.val_S2 = Wire()
    s.elm_S2 = [ Wire(DataType) for _ in range(4) ]

    @s.update_ff
    def pipereg_S1S2():
      s.val_S2 <<= s.val_S1 if ~s.reset else b1(0)
      for i in range(4):
    ...
    s.elm_next_S2 = [ Wire(DataType) for _ in range(4) ]
    ...
    s.val_S3 = Wire()
    s.elm_S3 = [ Wire(DataType) for _ in range(4) ]

    @s.update_ff
    def pipereg_S2S3():
      s.val_S3 <<= s.val_S2 if ~s.reset else b1(0)
      for i in range(4):
    ...
    s.elm_next_S3 = [ Wire(DataType) for _ in range(4) ]
